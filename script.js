...
document.addEventListener(
    ...
    () => {Alpine.data(
        ...
        () => ({
          ...
          currentUser: {login: ''},
          authModalOpen: false,
          authTab: 'login',  // 'login' или 'register'
          authForm: {
            login: '',
            password: '',
          },
          ...
          submitLogin() {
            ...
            this.currentUser = {
              name: this.authForm.login.split('@')[0] || 'Пользователь',
          ...
          updateAssetPriority(assetId, newPriority) {
            ...
            console.log(`✅ Актив "${asset.name}": приоритет ${oldPriority} → ${
                newPriority}. Пересчитано рисков: ${recalculatedCount}`);
          ...
          deleteAsset(id) {
            ...
            if (linkedRisks.length > 0) {
              const confirm_delete = confirm(`Актив "${asset.name}" имеет ${
                  linkedRisks
                      .length} риск(ов). Они будут удалены. Продолжить?`);
          ...
          submitRisk() {
            ...
            this.risks.push({
              ...
              score: this.calculateRisk(
                  this.riskForm.damage, this.riskForm.probability,
                  asset.priority),
              residualScore: null,
              measureId: null,
              reduceDamage: 0,
          ...
          submitMeasure() {
            if (!this.measureForm.name.trim()) {
              this.showNotification(
                  '❌ Введите название защитной меры', 'error');
              return;
            }
          ...
          confirmLinkMeasure() {
            ...
            const measure = this.measures.find(
                m => m.id === this.currentMeasureIdForLinking);
            if (!measure) {
          ...
          unlinkMeasureFromRisk(riskId) {
            ...
            const measureName =
                this.measures.find(m => m.id === risk.measureId) ?.name ||
                'мера';
