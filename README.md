...
Текущий этап завершён: серверная часть уже работает с JWT, Prisma и PostgreSQL, а клиент подготовлен к работе через Axios API. Дальнейшая доработка фронтенда сводится к полной замене оставшихся вызовов к старой локальной БД на API-вызовы без изменения математической логики UI.
